...
export default function HomePage() {
  ...
  const load = useCallback(async () => {
    ...
    eggs?.forEach((r) => {
      todayEggs[r.room_id] = (todayEggs[r.room_id] || 0) + r.count
    })

    // 餌（今日・部屋ごと合計）
    const { data: feed } = await supabase
      .from('feed_records')
      .select('room_id, amount_kg')
      .eq('record_date', currentDate)
    ...
    // 羽数（設定 - 入雛日以降の累計死鶏）
    const { data: flockSettings } = await supabase
      .from('flock_settings')
      .select('room_id, start_date, initial_count')
    ...
    if (flockSettings && flockSettings.length > 0) {
      ...
      flockSettings.forEach((fs) => {
        ...
        currentFlock[fs.room_id] = fs.initial_count - dead
      })
    }
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rooms.map((room) => {
                  ...
                  return (
                    <tr key={room.id} className="border-b border-border/50 last:border-0">
                      <td className="py-2.5 text-xs font-bold text-text2">{room.name}</td>
                      <td className={`py-2.5 text-center font-bold ${eggs != null ? 'text-accent' : 'text-border'}`}>
                        {eggs != null ? `${eggs}個` : '－'}
                      </td>
                      <td className={`py-2.5 text-center font-bold ${feed != null ? 'text-green' : 'text-border'}`}>
                        {feed != null ? `${feed}kg` : '－'}
                      </td>
                      {hasFlock && (
                        <td className={`py-2.5 text-center font-bold ${flock != null ? 'text-blue' : 'text-border'}`}>
                          {flock != null ? `${flock}羽` : '－'}
                        </td>
                      )}
                    </tr>
                ...
                <tr className="border-t-2 border-border font-black">
                  ...
                  <td className="pt-2.5 text-center text-accent">{totalEggs}個</td>
                  <td className="pt-2.5 text-center text-green">{totalFeed}kg</td>
                  {hasFlock && <td className="pt-2.5 text-center text-blue">{totalFlock}羽</td>}
                </tr>
              </tbody>
            </table>
          )}
